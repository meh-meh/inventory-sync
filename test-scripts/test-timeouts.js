...
async function testTimeouts() {
	try {
		...
		try {
			...
			logger.info(
				`✅ Operation completed in ${endTime - startTime}ms with result: ${productCount}`
			);
		} catch (error) {
		...
		try {
			...
			const endTime = Date.now();
			logger.info(
				`✅ Complex query completed in ${endTime - startTime}ms with ${result.length} results`
			);
		} catch (error) {
		...
		try {
			...
			const endTime = Date.now();
			logger.info(
				`Query unexpectedly completed in ${endTime - startTime}ms with ${result.length} results`
			);
		} catch (error) {
