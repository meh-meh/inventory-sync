	...
	try {
		...
		const recommendedIndexes = {
			products: [
				...
				{
					fields: { 'etsy_data.listing_id': 1 },
					options: { sparse: true, name: 'etsy_listing_id' },
				},
				{
					fields: { 'shopify_data.product_id': 1 },
					options: { sparse: true, name: 'shopify_product_id' },
				},

				// Fields used in frequently used queries
				{
					fields: { quantity_on_hand: 1, quantity_committed: 1 },
					options: { name: 'inventory_levels' },
				},
				{ fields: { name: 'text' }, options: { name: 'name_text' } },

				// Fields used for sorting and dashboard
				{
					fields: { 'etsy_data.last_synced': -1 },
					options: { sparse: true, name: 'etsy_last_synced' },
				},
				{
					fields: { 'shopify_data.last_synced': -1 },
					options: { sparse: true, name: 'shopify_last_synced' },
				},
			],
			...
			orders: [
				...
				{
					fields: { shopify_order_number: 1 },
					options: { sparse: true, name: 'shopify_order_number' },
				},

				// Common query combinations
				{ fields: { marketplace: 1, status: 1 }, options: { name: 'marketplace_status' } },
				{
					fields: { status: 1, 'items.is_digital': 1 },
					options: { name: 'status_item_type' },
				},

				// Fields used for sorting and filtering
				{ fields: { order_date: -1 }, options: { name: 'order_date_desc' } },
				{
					fields: { shipped_date: -1 },
					options: { sparse: true, name: 'shipped_date_desc' },
				},
		...
		for (const [collectionName, indexes] of Object.entries(recommendedIndexes)) {
			...
			for (const indexDef of indexes) {
				try {
					...
					const modelName =
						collectionName.charAt(0).toUpperCase() + collectionName.slice(1, -1); // Remove last 's'
					...
					logger.info(
						`Successfully created index ${indexDef.options.name} on ${collectionName}`
					);
				} catch (error) {
					logger.error(
